...
- `electron/main.js` sceglie la cartella condivisa col picker nativo (persistita in
  `userData/mvn-config.json`), e serve il renderer da `app://mvn` (origine stabile per
  `localStorage` = ricordare "chi sei"). I/O fs con scrittura atomica (temp+rename).
